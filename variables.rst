...
Julia REPLおよび他のJuliaの編集環境では、バックスラッシュを伴うLaTex記号とタブを入力することでUnicode数学記号を入力することが可能です。例えば、変数名 ``δ`` は、``\delta``-tabとすることで入力できます。また、 ``α̂₂`` は、``\alpha``-tab-``\hat``-tab-``\_2``-tabとすることで入力できます。
